 ...
 * unit-tested `addRasterRaise` below.
 */
...
/**
 * Pure pixels→heights step: add a signed, whole-metre height delta to each cell from a painted
 * coverage raster. `image` is RGBA bytes (row-major, length `heights.length * 4`); the red channel
 * is the coverage (0..255 → 0..1). The delta is `round(raise · coverage)`, so:
